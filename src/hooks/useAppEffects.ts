...
export function useAppEffects({ auth, vehicles, maintenance, navigation }: UseAppEffectsProps) {
  ...
  // Após login/logout/login, o app deve voltar para "Meus veículos",
  // mesmo quando ainda não existe nenhum veículo cadastrado.
  useEffect(() => {
    if (!auth.currentUser || vehicles.isLoading) return;

    const screensThatCanStayOpen = ['registration', 'dashboard', 'add-item', 'record-maintenance', 'history', 'update-km'];

    if (!vehicles.selectedVehicleId && !screensThatCanStayOpen.includes(navigation.currentScreen)) {
      navigation.navigateTo('vehicle-selection');
    }
  }, [auth.currentUser, vehicles.selectedVehicleId, vehicles.isLoading, navigation.currentScreen]);
